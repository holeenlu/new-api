...
- 本地并发保护在 HTTP/SSE、Responses WebSocket 和 Alpha Search 路径统一返回专用、可重试的
  `oauth_channel_concurrency_limit` 503。并发槽饱和时使用 `Retry-After: 1`；普通冷却返回向上取整后的真实剩余秒数；套餐用量窗口冷却会保留 `upstream_usage_limit` 原因：Claude 推理响应同时给出 5 小时和周窗口状态/重置时间时，客户端会分别显示每个已耗尽窗口，且凭证冷却到较晚的重置时间；仅给出一个窗口时只显示该窗口；缺少可靠窗口信息时才回退通用提示。Claude 200 SSE 在响应头已确认窗口耗尽时会立即关闭上游正文并故障转移，不等待首事件 30 秒超时；已输出 Claude SSE 后收到终止错误会发送 `event: error` 并结束，绝不向已提交的流附加 JSON；达到请求级 OAuth 尝试预算时立即返回最后一个真实错误，不会在候选选择循环中让客户端持续等待。该错误只写运行日志，不进入持久化错误日志；
...
- Claude Code 不再调用或暴露 `https://api.anthropic.com/api/oauth/usage`；该接口可能独立返回 `403/429`，也无法稳定提供准确用量窗口和重置时间，不能作为推理失败原因的可靠证据。推理重试只使用实际模型响应的 HTTP 状态、结构化错误代码/类型、错误信息和 `Retry-After`，以及被动返回的 `anthropic-ratelimit-unified-5h-status` / `-5h-reset`、`-7d-status` / `-7d-reset` 响应头：通用 `429 rate_limit_error` 及 `This request would exceed your account's rate limit` 归类为 `upstream_rate_limited`；只要任一窗口状态或顶层 `anthropic-ratelimit-unified-status: rejected` 明确耗尽，即使没有 reset，也归类为 `upstream_usage_limit`，以一小时冷却并向客户端明确说明上游未提供恢复时间；
