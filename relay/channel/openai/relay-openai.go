...
func OaiStreamHandler(c *gin.Context, info *relaycommon.RelayInfo, resp *http.Response) (*dto.Usage, *types.NewAPIError) {
	...
	if err := handleLastResponse(lastStreamData, &responseId, &createAt, &systemFingerprint, &model, &usage,
		...
		logger.LogError(c, fmt.Sprintf("error handling last response: %s; final SSE event content omitted (%d bytes)", err.Error(), len(lastStreamData)))
	}
...
func OpenaiHandler(c *gin.Context, info *relaycommon.RelayInfo, resp *http.Response) (*dto.Usage, *types.NewAPIError) {
	...
	if info.ChannelType == constant.ChannelTypeOpenRouter && info.ChannelOtherSettings.IsOpenRouterEnterprise() {
		...
		if enterpriseResponse.Success {
			...
			logger.LogError(c, "openrouter enterprise response reported success=false; response content omitted")
