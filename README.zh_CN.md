...
| `DEFAULT_LANGUAGE` | 客户端未携带受支持的 `Accept-Language` 时，本地化 API 响应（如错误信息）使用的默认语言；用户显式设置与 `Accept-Language` 仍优先。可取 `en`、`zh`、`zh-CN`、`zh-TW` | `en` |
...
订阅 OAuth 渠道会冻结请求首次选中的实际分组，并且只在该分组内的数据策略兼容渠道间重试；渠道标签仅作为管理元数据，不参与订阅渠道路由。其他渠道类型继续使用已配置的重试隔离规则，包括相同标签自动隔离。管理员可以在渠道编辑器的“数据治理”中选择更严格的隔离范围。
...
供应商和数据策略值是管理员声明的元数据，不是供应商签名或认证的承诺；启用跨渠道重试前，应根据上游账户合同核验这些值。
