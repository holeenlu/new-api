...
import {
  ...
  OFFICIAL_ANTHROPIC_PRICING_ENDPOINT,
  OFFICIAL_ANTHROPIC_PRICING_ID,
  OFFICIAL_CHANNEL_ENDPOINT,
  OFFICIAL_CHANNEL_ID,
  OFFICIAL_OPENAI_PRICING_ENDPOINT,
...
function getDefaultEndpointForChannel(channel: UpstreamChannel): string {
  ...
  if (channel.id === OFFICIAL_OPENAI_PRICING_ID) {
    return OFFICIAL_OPENAI_PRICING_ENDPOINT
  }
  if (channel.id === OFFICIAL_ANTHROPIC_PRICING_ID) {
    return OFFICIAL_ANTHROPIC_PRICING_ENDPOINT
  }
...
export function UpstreamRatioSync({ modelRatios }: UpstreamRatioSyncProps) {
  ...
  const { mutateAsync: syncMutateAsync, isPending: isSyncPending } =
    useMutation({
      mutationFn: async (updates: Array<{ key: string; value: string }>) => {
        const result = await updateModelPricingOptions({
  ...
  const performSync = useCallback(
    async (currentRatios: ParsedRatios): Promise<boolean> => {
      ...
      try {
        await syncMutateAsync(updates)
        return true
      } catch {
        return false
      }
    },
    [resolutions, syncMutateAsync]
  )
