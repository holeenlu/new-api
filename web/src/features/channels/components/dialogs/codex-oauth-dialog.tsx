...
export function CodexOAuthDialog(props: CodexOAuthDialogProps) {
  ...
  const canComplete = useMemo(
    () => Boolean(callbackUrl.trim()) && Boolean(flowToken) && !isCompleting,
    [callbackUrl, flowToken, isCompleting]
  )
  ...
  const handleStart = async (): Promise<void> => {
    ...
    try {
      ...
      const nextFlowToken = response.data?.flow_token
      if (!response.success || !url || !nextFlowToken) {
  ...
  const handleComplete = async (): Promise<void> => {
    ...
    try {
      const response = await completeCodexOAuth(callbackUrl.trim(), flowToken)
