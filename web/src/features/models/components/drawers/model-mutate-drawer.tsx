...
function readPricingConfig(
  ...
  const pricingInputMode = safeJsonParse<Record<string, PricingSubMode>>(
    settings.ModelPricingInputMode,
  ...
  return {
    mode: 'per-token',
    subMode: pricingInputMode === 'price' ? 'price' : 'ratio',
    fields: {
      price: '',
...
export function ModelMutateDrawer({
  ...
  const modelSettings = useMemo(() => {
    ...
    const defaultModelSettings: ModelSettings = {
      ...
      SubscriptionOAuthUpstreamRetryTimes: 5,
      SubscriptionOAuthCapacityCycleTimes: 5,
      SubscriptionOAuthCapacityWaitSeconds: 5,
      SubscriptionOAuthRetry429: false,
    }
  ...
  useEffect(() => {
    if (open && isEditing && modelData?.data) {
      ...
      setPricingSubMode(pricing.subMode)
        ...
        if (response.success) {
          ...
          if (modelSettings) {
            ...
            if (
              ...
              updates.push({
                key: 'ModelPricingInputMode',
                value: newPricingInputMode,
              })
            }
