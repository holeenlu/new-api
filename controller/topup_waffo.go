...
func RequestWaffoPay(c *gin.Context) {
	...
	if !resp.IsSuccess() {
		logger.LogWarn(c.Request.Context(), fmt.Sprintf("Waffo 创建订单业务失败 user_id=%d trade_no=%s code=%s", id, merchantOrderId, resp.Code))
		topUp.Status = common.TopUpStatusFailed
		_ = topUp.Update()
		c.JSON(http.StatusOK, gin.H{"message": "error", "data": "拉起支付失败"})
		return
...
func WaffoWebhook(c *gin.Context) {
	...
	logger.LogInfo(c.Request.Context(), fmt.Sprintf("Waffo webhook 收到请求 path=%q client_ip=%s body_size=%d", c.Request.RequestURI, c.ClientIP(), len(bodyStr)))

	// 验证请求签名
	if !wh.VerifySignature(bodyStr, signature) {
		logger.LogWarn(c.Request.Context(), fmt.Sprintf("Waffo webhook 验签失败 path=%q client_ip=%s body_size=%d", c.Request.RequestURI, c.ClientIP(), len(bodyStr)))
		c.AbortWithStatus(http.StatusBadRequest)
		return
	}
	...
	if err := common.Unmarshal(bodyBytes, &event); err != nil {
		logger.LogError(c.Request.Context(), fmt.Sprintf("Waffo webhook 解析失败 path=%q client_ip=%s error=%q body_size=%d", c.Request.RequestURI, c.ClientIP(), err.Error(), len(bodyStr)))
	...
	case core.EventPayment:
		...
		if err := common.Unmarshal(bodyBytes, &payload); err != nil {
			logger.LogError(c.Request.Context(), fmt.Sprintf("Waffo 支付回调载荷解析失败 event_type=%s client_ip=%s error=%q body_size=%d", event.EventType, c.ClientIP(), err.Error(), len(bodyStr)))
