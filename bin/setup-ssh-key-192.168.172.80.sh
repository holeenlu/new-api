...
DEPLOY_TARGET=${DEPLOY_TARGET:-kdanmobile@192.168.172.80}
...
if [[ ! -f "$DEPLOY_SSH_KEY" ]]; then
  ssh-keygen -q -t ed25519 -N '' -f "$DEPLOY_SSH_KEY" -C "new-api-deploy-cc00-ai"
fi
...
ASKPASS_SCRIPT=$(mktemp "${TMPDIR:-/tmp}/new-api-cc00-ai-askpass.XXXXXX")
