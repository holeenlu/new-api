...
| `DEFAULT_LANGUAGE` | Fallback language for localized API responses (e.g. error messages) when the client sends no supported `Accept-Language`. Explicit user settings and `Accept-Language` still take precedence. Accepts `en`, `zh`, `zh-CN`, `zh-TW` | `en` |
...
Provider and data-policy values are administrator-declared metadata, not provider-attested claims. Verify them against the upstream account contract before enabling cross-channel retry.
