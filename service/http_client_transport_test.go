...
func TestResponseHeaderTimeoutClientPreservesTransportPolicy(t *testing.T) {
	initDefaultHTTPClientFixture(t)
	const timeout = 250 * time.Millisecond

	http1Client, err := GetHttpClientWithResponseHeaderTimeoutSettings(
		"",
		dto.ChannelSettings{HTTPProtocol: dto.HTTPProtocolHTTP1},
		timeout,
	)
	require.NoError(t, err)
	http1Transport, ok := http1Client.Transport.(*http.Transport)
	require.True(t, ok)
	assert.Equal(t, timeout, http1Transport.ResponseHeaderTimeout)
	assert.False(t, http1Transport.ForceAttemptHTTP2)
	assert.NotNil(t, http1Transport.TLSNextProto)

	shardedClient, err := GetHttpClientWithResponseHeaderTimeoutSettings(
		"",
		dto.ChannelSettings{HTTP2ConnectionShards: 3},
		timeout,
	)
	require.NoError(t, err)
	shardedTransport, ok := shardedClient.Transport.(*shardedRoundTripper)
	require.True(t, ok)
	require.Len(t, shardedTransport.shards, 3)
	for _, shard := range shardedTransport.shards {
	...
	cachedClient, err := GetHttpClientWithResponseHeaderTimeoutSettings(
		"",
		dto.ChannelSettings{HTTP2ConnectionShards: 3},
		timeout,
	)
	require.NoError(t, err)
	assert.Same(t, shardedClient, cachedClient)

	differentTimeoutClient, err := GetHttpClientWithResponseHeaderTimeoutSettings(
		"",
		dto.ChannelSettings{HTTP2ConnectionShards: 3},
		500*time.Millisecond,
	)
	require.NoError(t, err)
	assert.NotSame(t, shardedClient, differentTimeoutClient)
}
...
func TestInvalidateProxyClientClosesAllPolicyVariants(t *testing.T) {
	...
	require.NoError(t, err)
	timeoutClient, err := GetHttpClientWithResponseHeaderTimeoutSettings(
		proxyURL,
		dto.ChannelSettings{HTTP2ConnectionShards: 2},
	...
	require.NoError(t, err)
	afterTimeout, err := GetHttpClientWithResponseHeaderTimeoutSettings(
		proxyURL,
		dto.ChannelSettings{HTTP2ConnectionShards: 2},
		250*time.Millisecond,
	)
	...
	assert.NotSame(t, timeoutClient, afterTimeout)
}
