...
func newHTTPClientFromPolicyWithResponseHeaderTimeout(
	policy HTTPTransportPolicy,
	proxyURL *url.URL,
	...
	return newHTTPClientFromTransportFactory(policy, func() *http.Transport {
		transport := factory()
...
func GetHttpClientWithResponseHeaderTimeout(proxyURL string, timeout time.Duration) (*http.Client, error) {
	return GetHttpClientWithResponseHeaderTimeoutSettings(proxyURL, dto.ChannelSettings{}, timeout)
}

// GetHttpClientWithResponseHeaderTimeoutSettings returns a cached timeout client
// keyed by canonical proxy URL, normalized channel transport policy, and timeout.
func GetHttpClientWithResponseHeaderTimeoutSettings(
	rawProxyURL string,
	settings dto.ChannelSettings,
	timeout time.Duration,
) (*http.Client, error) {
	if timeout <= 0 {
		return GetHttpClientWithProxySettings(rawProxyURL, settings)
	}

	policy := NormalizeHTTPTransportPolicy(settings)
	trimmedProxyURL := strings.TrimSpace(rawProxyURL)
	canonicalKey := ""
	var config *proxyURLConfig
	if trimmedProxyURL != "" {
		parsedURL, legacySuffixStripped, err := common.ParseProxyURLRuntime(trimmedProxyURL)
		if err != nil {
			return nil, err
		...
		config = newProxyURLConfig(parsedURL)
		canonicalKey = config.cacheKey
		if legacySuffixStripped {
			warnLegacyProxyURLOnce(config)
		}
	}

	cacheKey := clientCacheKey(canonicalKey, policy) + "\x00" + timeout.String()
	responseTimeoutLock.Lock()
	...
	if config != nil {
		parsedProxyURL = config.parsedURL
	}
	client, err := newHTTPClientFromPolicyWithResponseHeaderTimeout(policy, parsedProxyURL, timeout)
	if err != nil {
		return nil, err
	}
	responseTimeoutClients[cacheKey] = client
	return client, nil
}
